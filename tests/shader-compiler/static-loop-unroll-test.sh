...
refusal_status "$rc" "dynamic loop"
[[ "$rc" -eq 1 ]] || fail "dynamic loop compiled; data-dependent fragment loops must still refuse"
...
refusal_status "$rc" "short-circuit sqrt comparison"
[[ "$rc" -eq 0 && -s "$out" ]] || {
    tail -n 20 "$log" >&2
