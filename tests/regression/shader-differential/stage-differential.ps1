...
param(
    ...
    # ours-vs-reference rows, independent of the retired legacy matcher.
    # This is the reference-backed successor to gate1's legacy/general
    # tree arm.  Defaults to the repo root plus path-pair-corpus.txt.
    [switch]$ReferenceTreeCorpus,
    [string]$ReferenceTreeCorpusDir = "",
    [string]$ReferenceTreeCorpusManifest = "",
...
# Second texture control (2026-09-02): the same twin pair sampled through
# TEXCOORD1 = (u-0.5, v-0.5), a window half outside 0..1, against a
# frac-based arithmetic twin.  The guest addresses every texture WRAP so
# that a shader sampling such a window (th06's uv) reads every texel; this
# row proves wrapped sampling lands where the arithmetic says.  Same role,
# same gate: a mismatch withholds every sampler-declaring verdict.
Print-AutoTextures (Join-Path $here "shaders\sd_tex_wrap_ctrl.fcg") "control-texture (wrap)"
Compile-Shader "sd_tex_wrap_ctrl.fcg"  (Join-Path $controls "tex_wrap.fpo")       @("--general-lowering")
Compile-Shader "sd_tex_wrap_baked.fcg" (Join-Path $controls "tex_wrap_baked.fpo") @("--general-lowering")
...
$manifest = @(
    ...
    "# fields: tier|role|name|a_path|b_path|uniform_set[|blind-ok][|vp=<path>][|judge=<c1,c2,c3,z>]  (blind-ok = sweep row: discard-blind is inventory, not a gate failure; vp= = row-private vertex program; judge= = control-* rows only, judge outputs neither side declares, same compiler both sides)",
    "@target emulator",
    ...
    "B|control-texture|texture_wrap|controls/tex_wrap.fpo|controls/tex_wrap_baked.fpo|0",
...
if ($ReferenceTreeCorpus -and -not $ReferenceCompiler) {
    throw "-ReferenceTreeCorpus needs the reference compiler (pass -ReferenceCompiler <exe> or set PS3_REF_CG_COMPILER)"
}
if ($ReferenceCompiler) {
    ...
    if ($ReferenceCorpus) {
        ...
        foreach ($f in $files) {
            ...
            Copy-Item $ours (Join-Path $refDst "$name`_ours.fpo") -Force
            Copy-Item $ref  (Join-Path $refDst "$name`_ref.fpo") -Force
            $cRows += "B|reference|$name|reference/$name`_ours.fpo|reference/$name`_ref.fpo|auto|blind-ok"
        }
    ...
    # Reference tree corpus: the tracked gate1 tree input set, but judged
    # against the reference compiler instead of the retired legacy matcher.
    if ($ReferenceTreeCorpus) {
        if (-not $ReferenceTreeCorpusDir) { $ReferenceTreeCorpusDir = $repoRoot }
        if (-not (Test-Path -LiteralPath $ReferenceTreeCorpusDir -PathType Container)) {
            throw "reference tree corpus root not a directory: $ReferenceTreeCorpusDir"
        }
        $rtRoot = (Resolve-Path -LiteralPath $ReferenceTreeCorpusDir).Path.TrimEnd('\')
        ...
        $rtFileRows = @(Get-ReferenceTreeCorpusFiles -Root $rtRoot -Manifest $ReferenceTreeCorpusManifest)
        if ($rtFileRows.Count -eq 0) { throw "reference tree corpus is empty: $rtRoot" }

        $rtRefusedPath = Join-Path $root "reference-tree-corpus-refused.txt"
        $rtRefusedRows = @("# shader-differential reference tree corpus refusals -- generated by stage-differential.ps1",
                           "# fields: name|side|repo-relative path|rc. side=ours is an accepted-reference shader our default/general path refused; side=reference is outside the reference-backed gate.")
        $rtRows = @(); $rtIdentical = 0; $rtRefusedOurs = 0; $rtRefusedRef = 0; $rtBothRefused = 0; $rtProbed = 0
        $rtAttributions = @()
        foreach ($fileRow in $rtFileRows) {
            $f = $fileRow.File
            ...
            if ($rel -eq "tests/regression/shader-readback/shaders/rb_refract_k0.fcg") {
                ...
                $rtAttributions += "accept_array_uniform=new-finding(uniform array row compiles on both sides but mismatches pixels under auto binding)"
            }
            if (-not $okOurs) {
                $rtRefusedOurs++; $rtRefusedRows += "$name|ours|$rel|$rcOurs"
                if ($ReferenceProbeRefused -and $okRef) {
                    Copy-Item $ref (Join-Path $refDst "$name`_rtref.fpo") -Force
                    $rtRows += "B|probe|$name`_rtrefonly|reference/$name`_rtref.fpo|reference/$name`_rtref.fpo|$rtSet"
                    $rtProbed++
                }
            }
            if (-not $okRef) { $rtRefusedRef++; $rtRefusedRows += "$name|reference|$rel|$rcRef" }
            if (-not $okOurs -and -not $okRef) { $rtBothRefused++ }
            if (-not ($okOurs -and $okRef)) { continue }
            $hOurs = (Get-FileHash -Algorithm SHA256 -LiteralPath $ours).Hash
            $hRef  = (Get-FileHash -Algorithm SHA256 -LiteralPath $ref).Hash
            Add-ContainerMetricsRow ([ref]$containerMetricRows) -Name $name `
                ...
                -ByteIdentical:($hOurs -eq $hRef) -Staged:($hOurs -ne $hRef)
            if ($hOurs -eq $hRef) { $rtIdentical++; continue }
            Copy-Item $ours (Join-Path $refDst "$name`_rtours.fpo") -Force
            Copy-Item $ref  (Join-Path $refDst "$name`_rtref.fpo") -Force
            $rtRows += "B|reference|$name|reference/$name`_rtours.fpo|reference/$name`_rtref.fpo|$rtSet|blind-ok"
        }
        Set-Content -LiteralPath $rtRefusedPath -Value ($rtRefusedRows -join "`n") -Encoding Ascii
        if (($rtRows.Count + $rtIdentical + $rtRefusedOurs + $rtRefusedRef) -eq 0) { throw "reference tree corpus sweep compiled nothing" }
        $manifest += $rtRows
        Write-Host (Format-ReferenceTreeCorpusSummary `
            -CandidateCount $rtFileRows.Count `
            -OursRefused $rtRefusedOurs `
            -ReferenceRefused $rtRefusedRef `
            -BothRefused $rtBothRefused `
            -ByteIdentical $rtIdentical `
            -PairsStaged ($rtRows.Count - $rtProbed) `
            -ProbeRows $rtProbed)
        if ($rtAttributions.Count -gt 0) {
            Write-Host ("stager: reference tree non-green attribution: {0}" -f (($rtAttributions | Sort-Object -Unique) -join "; "))
        }
    }
...
if ($VpPairs -or $VpCorpus -or $VpPathPairs) {
    ...
    # The channels the shared VP does NOT write - fog, col1, tc4..tc9 - get
    # their proof row under sd_pos_allch_ext.vcg, which writes every channel
    # the rig judges.  REFERENCE-compiled (an instrument must not rest on
    # the compiler under test) and staged as a runtime vertex program the
    # guest binds for that row alone (manifest field vp=<path>).  If the
    # reference refuses it the extended rows are simply not staged and the
    # stage says so; nothing else changes.
    $vpExt = Join-Path $controls "vp_ext.vpo"
    $vpExtOk = Compile-Reference (Join-Path $here "shaders\sd_pos_allch_ext.vcg") $vpExt "sce_vp_rsx"
    if ($vpExtOk) { Write-Host "stager: extended instrument VP staged (reference-compiled): fog, col1, tc4..tc9 coverage FPs get proof rows under it" }
    else { Write-Host "stager: extended instrument VP did not compile on the reference - fog/col1/tc4..tc9 coverage FPs stay unproven" }
    $vpCovMissing = @(); $vpCovRows = @(); $vpCovIdentical = 0; $vpCovExtRows = 0
    foreach ($c in $vpChannels) {
        ...
        if ($c.key -notin $vpProvable) {
            if (-not $vpExtOk) { continue }
            Copy-Item $covRef (Join-Path $controls "vpcov_$($c.key)_ref.fpo") -Force
            $vpCovRows += "B|reference|vpcov_$($c.key)@instrument|controls/vpcov_$($c.key).fpo|controls/vpcov_$($c.key)_ref.fpo|0|vp=controls/vp_ext.vpo"
            $vpCovExtRows++
            continue
        }
        Copy-Item $covRef (Join-Path $controls "vpcov_$($c.key)_ref.fpo") -Force
    ...
    $manifest += $vpCovRows
    Write-Host "stager: vp coverage FPs staged ($($vpChannels.Count - $vpCovMissing.Count) of $($vpChannels.Count): $vpCovIdentical byte-identical to the reference, $($vpCovRows.Count - $vpCovExtRows) judged on pixels under the shared VP, $vpCovExtRows under the extended VP)"
