...
- **FP — compound-discard lowering.**  Two-pass emit (analysis + emit)
  ...
  output) now byte-match the reference compiler at the full container level.
- **FP — dual-texture discard ordering.**  When a fragment program
  ...
  schedule the reference compiler produces.
- **FP — `VecConstructTexMul` split-write shape.**  Recognises
  ...
  instead of a single MOV, matching the reference compiler's decomposition of
...
The byte-diff suite against `the reference compiler` at `--O2 --fastmath` grows from
...
- **Repeated-add scale-fold (full vec4)** generalises the pattern
  ...
  `the reference compiler` switches to a DP2R shape there which is documented in
  `KNOWN_ISSUES.md`.
...
- **Tools:**
  ...
  - `rsx-cg-compiler` — Cg → RSX (NV40) compiler.  46/46 byte-match
    against `the reference compiler` for the stage-4 test corpus.
- **ABI:**
...
- **Reference-SDK builds:** unmodified `basic.cpp` (RSX triangle) and
  ...
  this SDK using stock `the reference compiler` shaders, and run in RPCS3.
