...
`<library>.yaml`, matching the the reference SDK's library prefix:
...
Signatures are reimplemented from the Sony header text by independent
paraphrase, not copy.

The the reference SDK mount at `the reference SDK/` is never committed.
