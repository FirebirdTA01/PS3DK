...
How `rsx-cg-compiler` models `the reference compiler`'s compile-time knobs, and where
future optimization passes plug in.

## Today's reality (probed 2026-04-18, SDK the reference SDK)

Of the ~60 flags `the reference compiler --help` documents, exactly **one** actually
...
`namespace rsx_cg`.  Defaults match the reference compiler's defaults so the
byte-diff harness compares like-for-like without either side passing
extra flags.

```cpp
struct CompileOptions {
    OptLevel opt      = OptLevel::O2;   // the reference compiler default
    bool     fastmath = true;           // default ON
    bool     backFaceColorBits = false; // attributeOutputMask: front-face only
    bool     keepUnusedParams = false;  // the reference compiler strips unused by default
    bool     useNrmh           = true;
...
it's equivalent to running the reference compiler with no flags.
...
the reference compiler's code-gen flags (`--bcolor`, `--fkeep-unused`, `--fuse-nrmh`,
...
4. Add a test shader that exercises the difference.  Give it a
   ...
   invokes the reference compiler with matching flags.

**Hard rule:** every new optimization must still produce byte-exact
output against the reference compiler at the same `(OptLevel, fastmath)` tuple.
Our diff harness is the gate; optimizations that pass functional
tests but diverge from the reference compiler are bugs.

## The test contract

`tests/run_diff.sh` pins against `--O2 --fastmath` on **both** sides
(our compiler and the reference compiler).  This is documented at the top of the
...
Every shader in `tests/shaders/` today is simple enough that the reference compiler
produces identical output at `--fastmath` and `--nofastmath`.  We
match the reference compiler at `--fastmath` by construction because that's what
the reference compiler emits as the baseline.  When we land the first shader that
DOES differ (e.g. a 3-term `MUL+ADD` expression the reference compiler fuses into a
