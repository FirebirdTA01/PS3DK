 ...
 *   - Matches the reference compiler's observed layout — component order, rows descending.
 *
 * the reference compiler's allocator + ucode CONST_SRC encoding are documented in
...
// result value is consumed by a StoreOutput (matches the reference compiler which writes
