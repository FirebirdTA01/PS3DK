...
//
// Small on purpose.  The point of the first conversion is to prove the
// MECHANISM end to end under the byte fence: a [&] lambda becomes a free
// function, its capture becomes a parameter, and the emitted bytes do not
// move.  Proving that on a twelve-line pure walk is worth more than
// attempting it on a 100-line matcher, because when the fence passes here it
// is evidence about the transport rather than about one matcher's rewrite.
//
                                 ...
                                 FpAttributes* attrsOut)
{
    ...
    // sampler IR value → texture-unit index.  the reference compiler assigns tex
    ...
    // output register.  Per the reference compiler byte probes:
    //   - ADD emits src0=CONST(uniform), src1=INPUT(varying)
    //   - MUL emits src0=INPUT(varying), src1=CONST(uniform)
    //   (operand order in C source is ignored — the reference compiler canonicalises)
    // Per the reference compiler byte probes, all (input, uniform) binary opcodes
    ...
    // either a literal vec4 or another uniform.  the reference compiler lowers this
    ...
    //
    // Scheduling rule (verified from the reference compiler byte probes):
    ...
    //
    // Combining rules matching the reference compiler (and standard Cg semantics):
    ...
    //   `(vcol.x > 0.5f) ? a : b`
    //
    // the reference compiler lowers this to three instructions + two inline blocks:
    ...
    // 16-byte inline const block (matches the reference compiler's `FENCBR; MOVR R0,
    ...
    // output lanes.  the reference compiler emits the 2-insn shape:
    ...
    // `length(vec3)` — the reference compiler lowers to MOVH + DP3R + DIVSQR + MOVR.
    ...
    for (const auto& blockPtr : entry.blocks)
    {
        ...
        for (const auto& instPtr : blockPtr->instructions)
        {
            ...
            case IROp::Discard:
            {
                ...
                // the reference compiler emits compound-discard comparisons in reverse
                ...
                // the reference compiler's schedule where TEX always comes first,
                ...
                // should precede the next TEX (the reference compiler dual-discard
                // pattern).
                ...
                for (const auto& rc : rcmps)
                {
                    ...
                    case CmpBinding::LhsKind::TexResult:
                    {
                            ...
                            if (txIt != valueToTex.end())
                            {
                                ...
                                if (sampIt != valueToTexUnit.end())
                                {
                                    ...
                                    if (uvIt != valueToInputSrc.end())
                                    {
                                        ...
                                        // the discard comparison, the reference compiler uses a
                        ...
                        {
                            cmpInsn.cc_update = 1;
                            cmpInsn.precision = 2; // FX12 matches the reference compiler
                        }
                        asm_.emit(cmpInsn, cb.opcode);
                ...
                if (cmpIds.size() >= 2)
                {
                    ...
                    mulxc.cc_update = 1;
                    // the reference compiler encodes MULXC with precision=FX12(2)
                    mulxc.precision = 2;
            ...
            case IROp::Div:
            {
                // Recognise `1.0 / <scalar>` as reciprocal — the reference compiler
            ...
            case IROp::VecConstruct:
            {
                ...
                // emits the reference compiler's MOVH/DP3/MOVR-w/DIVSQR sequence.
                if (inst.operands.size() == 2)
                {
                ...
                // plus one (varying, tex) Mul lane.  the reference compiler emits:
                ...
                // the reference compiler accumulator-fold output shape (see
            ...
            case IROp::StoreOutput:
            {
                ...
                // straight to the output register (the reference compiler's pattern —
                ...
                if (texIt != valueToTex.end())
                {
                    ...
                    // 3-component direction, not a 2D (u,v).  the reference compiler
                ...
                //
                // the reference compiler shape (probed against if_then_2add /
                ...
                if (pcIt != valueToPredCarry.end())
                {
                    ...
                    for (int i = 0; i < N; ++i)
                    {
                        ...
                        // chain has >= 2 links (matches the reference compiler's
                        ...
                        // original IR operand order — the reference compiler doesn't
                ...
                if (selIt != valueToSelect.end())
                {
                    ...
                    // Both-literal the reference compiler uses a swizzle-share trick
                    // we don't reproduce yet.  Literal trueBr plus any
                    // other branch is fine — the reference compiler just lays the
                    ...
                    // ============ Varying-RHS path ============
                    // The reference compiler reshapes the compare for a varying RHS
                    ...
                    // specifically, the reference compiler keeps the default identity
                    ...
                    //      nv40_if_convert), match the reference compiler's if-else
                ...
                if (sdmIt != valueToScaledDotsMinusLitMulUniWrap.end())
                {
                    ...
                    auto emitDot = [&](int outLane, const float* lit,
                        ...
                        // .yzzw cases the reference compiler replicates lane 0 of the
                ...
                //     DP3R R0.x, H0, H0;
                //     DIVSQR R0.x, |R0|, R0;       ; the reference compiler's sqrt trick
                ...
                if (lenIt != valueToLength.end())
                {
                    ...
                    // src0 is what makes the reference compiler's sqrt identity hold.
                ...
                if (nrmIt != valueToNormalize.end())
                {
                    ...
                    // final DIVSQR reads R0.z as src1 — matches the reference compiler)
                    const struct nvfx_reg r0 = nvfx_reg(NVFXSR_TEMP, 0);
                    struct nvfx_src dpS0 = nvfx_src(hDst);
                    struct nvfx_src dpS1 = nvfx_src(hDst);
                    struct nvfx_src dpS2 = nvfx_src(const_cast<struct nvfx_reg&>(none));
                    ...
                    // the reference compiler emits the disasm form `{...}.x` — SRC0 is
                ...
                // the reference compiler probes for 2-3 unique values): walk the
                ...
                // the reference compiler uses (inline.X is filler, real values land
                    ...
                    slIt->second.scaledLane != 0)
                {
                    ...
                    // 2-unique scaledLane=W has a shift-by-1 allocation
                    // the reference compiler uses; we don't implement it yet.  3-unique
                    // is fine because every slot gets used naturally.
                    if (sl.scaledLane == 3 && slotCount < 3)
                    {
                        out.diagnostics.push_back(
                            "nv40-fp: scaled-lanes scaledLane=W with < 3 "
                            "unique values not yet supported (the reference compiler shift-by-1)");
                        return out;
                    }
                    ...
                    // there's no "next" — the reference compiler uses W (slot 3 = filler).
                    if (sl.scaledLane == 3)
                ...
                // path) — the reference compiler accumulator-fold output shape.  Either
                ...
                //
                // Const packing follows the reference compiler's canonical order
                ...
                //
                // the reference compiler uses two distinct shapes depending on N:
                //
                //   N=2,3 (short shape):
                //     MOVH H0, f[input]
                ...
                // NO FENCBR here — the reference compiler only emits FENCBR before
                    ...
                    !shadowLikeShadowSplitShape(srcId))
                {
                    ...
                    if (bind.op == FpArithOp::Add)
                    {
                        ...
                        // ADD: src0=CONST, src1=INPUT (the reference compiler canonical)
                        slot0 = srcConst;
                        slot1 = srcIn;
                    }
                    else
                    {
                        ...
                        // verified against the reference compiler.
                        switch (bind.op)
                        {
