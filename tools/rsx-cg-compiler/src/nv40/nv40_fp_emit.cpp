    ...
    for (const auto& blockPtr : entry.blocks)
    {
        ...
        for (const auto& instPtr : blockPtr->instructions)
        {
            ...
            case IROp::StoreOutput:
            {
                ...
                // the the reference compiler accumulator-fold output shape.  3
                // instructions:
                //
                ...
                //
                // The MOVR uses the reference compiler's lane-packing trick: when
                    ...
                    slIt->second.scale.scale >= 2)
                {
                    ...
                    // 2-unique-values pattern matches the reference compiler's known
                    // packing today.
