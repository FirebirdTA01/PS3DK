 ...
 *
 * Models the subset of `the reference compiler` command-line flags that actually
 * change output bytes, plus room for ones that will matter once we
 * lower real-world shaders.  The default values MUST match the reference compiler's
 * defaults so our byte-exact diff harness compares like-for-like
 * without requiring extra flags on either side of the test.
 *
 * Today's reality check (probed against a 3.55-4.75-era reference SDK):
 *
 ...
 * output mask, which the reference compiler emits front-face-only by default), it
...
enum class OptLevel
{
    ...
    // yet implemented by the reference compiler for FP in the shaders we've probed):
    ...
    // `--O2` — the reference compiler default.  MAD fusion, register reuse across
...
struct CompileOptions
{
    ...
    // (the reference compiler docs say --bcolor is default ON but the reference compiler-output sets
    ...
    // the reference compiler strips them by default.
