 ...
 *         (immediately after the header — the reference compiler lays this out
 ...
 *
 * Strings are NOT deduplicated — the reference compiler emits a fresh copy of each
...
// The reference compiler's "kill marker" resource code — used on the synthetic
// $kill_NNNN parameter that #pragma alphakill <samplerName> produces.
// Not in the documented CG_BINDLOCATION_MACRO table; derived
// from the reference compiler output (see REVERSE_ENGINEERING.md "alphakill" section).
    ...
    const ContainerOptions&      opts)
{
    ...
    // the reference compiler: shader entry parameters, in declaration order. -----
    struct ParamDesc
    {
        std::string name;
        std::string semantic;
    ...
    // sampler (in source-order, which the reference compiler preserves and indexes
    // sequentially regardless of the original sampler's position).  These
    // params have no name/semantic in the source — the reference compiler generates the
    ...
    {
        ParamDesc d;
        // the reference compiler names these `$kill_<4-digit zero-padded index>`.
        ...
        d.name         = buf;
        d.semantic     = "";  // the reference compiler emits no semantic string for these
        d.type         = kCgFloat4;
        d.res          = kCgKillMarker;
        d.var          = kCgVarying;
        d.direction    = kCgOut;
    ...
    // form (matches the reference compiler).  Container is BE, ucode is treated as
    // raw u32 BE.
