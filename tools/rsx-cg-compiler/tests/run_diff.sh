#!/usr/bin/env bash
#
# Byte-diff rsx-cg-compiler output against Sony the reference compiler for every .cg in
...
# encodings that diverge from the reference compiler in subtle ways that don't show up
...
#     parameters) currently fall back to ucode-only diff: the reference compiler
...
SCE_CGC_EXE="$REPO_DIR/the reference SDK/host-win32/Cg/bin/the reference compiler.exe"
SHADER_DIR="$SCRIPT_DIR/shaders"
WORK_DIR="$(mktemp -d -t rsx-cg-compiler-diff.XXXXXX)"
trap 'rm -rf "$WORK_DIR"' EXIT

# Test contract: we pin against the reference compiler's DEFAULT optimisation level
...
if [[ ! -f "$SCE_CGC_EXE" ]]; then
    echo "error: the reference compiler.exe not found at $SCE_CGC_EXE" >&2
    echo "       mount the reference SDK (the reference SDK) and retry" >&2
    exit 1
fi
if ! command -v wine >/dev/null; then
    echo "error: wine not installed — required to run the reference compiler.exe" >&2
    exit 1
fi
...
# CgBinaryProgram layout (see the reference SDK/.../Cg/cgBinary.h):
#   0x00 profile
...
for cg in "$SHADER_DIR"/*.cg; do
    ...
    WINEDEBUG=-all wine "$SCE_CGC_EXE" --quiet --profile "$profile" \
        ...
        -o "$sce_out" "$cg" >"$WORK_DIR/${name}.sce.log" 2>&1 || {
            echo "[FAIL] $name: the reference compiler failed — see $WORK_DIR/${name}.sce.log" >&2
            overall_rc=1
            continue
        }
    ...
    # diff against the ucode region inside the reference compiler's container.
    ...
    if "$RSX_CG_COMPILER" --profile "$profile" \
       ...
       && [[ -s "$our_out" ]]; then
        if cmp -s "$our_out" "$sce_out"; then
            ...
            echo "[FAIL] $name: .${ext} container diverges from the reference compiler"
        ...
        if [[ "$our_hex" == "$sce_hex" ]]; then
            ...
            echo "[FAIL] $name: ucode diverges from the reference compiler"
            echo "       ours: $our_hex"
            echo "       sce : $sce_hex"
            overall_rc=1
        fi
...
echo
echo "$passed / $total shaders match the reference compiler byte-for-byte"
exit "$overall_rc"
