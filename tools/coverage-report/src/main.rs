//! coverage-report — the reference SDK vs PSL1GHT API coverage matrix.
//!
//! Walks `the reference SDK/target/{ppu,spu}/include/` and
...
//! with a libclang-based walker once the the reference SDK mount is available.
...
#[derive(Parser)]
#[command(name = "coverage-report", version, about = "the reference SDK vs PSL1GHT coverage matrix")]
struct Cli {
    /// Path to the reference SDK root (e.g. the reference SDK). If absent, run in
    /// PSL1GHT-only mode.
    #[arg(long)]
...
fn main() -> Result<()> {
    ...
    let sony_set = match cli.sony_sdk.as_ref() {
        ...
        _ => {
            eprintln!("note: running in PSL1GHT-only mode (no the reference SDK mount)");
            None
        }
    };
...
fn render_report(psl1ght: &SymbolSet, sony: Option<&SymbolSet>) -> String {
    ...
    let Some(sony) = sony else {
        writeln!(out, "## the reference SDK surface").ok();
        writeln!(out, "Not available — `the reference SDK/` is not mounted.").ok();
