/*
 * crt1.c — Lv-2 process C-entry glue.
 *
 * Replaces the libsysbase syscall-table wiring previously provided by
 * the upstream prefix.
 *
...
/* Reentrant syscall wrappers from librt.  Each maps to a slot in the
...
extern int        __librt_open_r(struct _reent *r, const char *file,
                                 int flags, int mode);
extern _ssize_t   __librt_read_r(struct _reent *r, int fd, void *ptr,
                                 size_t len);
extern _ssize_t   __librt_write_r(struct _reent *r, int fd,
                                  const void *ptr, size_t len);
extern _off_t     __librt_lseek_r(struct _reent *r, int fd, _off_t pos,
                                  int dir);
extern _off64_t   __librt_lseek64_r(struct _reent *r, int fd,
                                    _off64_t pos, int dir);
extern int        __librt_fstat_r(struct _reent *r, int fd,
                                  struct stat *st);
extern int        __librt_fstat64_r(struct _reent *r, int fd,
                                    struct stat *st);
extern int        __librt_stat_r(struct _reent *r, const char *path,
                                 struct stat *st);
extern int        __librt_stat64_r(struct _reent *r, const char *path,
                                   struct stat *st);
extern int        __librt_ftruncate_r(struct _reent *r, int fd,
                                      off_t len);
extern int        __librt_truncate_r(struct _reent *r, const char *path,
                                     off_t len);
extern int        __librt_fsync_r(struct _reent *r, int fd);
extern int        __librt_link_r(struct _reent *r, const char *old,
                                 const char *new);
extern int        __librt_unlink_r(struct _reent *r, const char *path);
extern int        __librt_chmod_r(struct _reent *r, const char *path,
                                  mode_t mode);
extern int        __librt_rename_r(struct _reent *r, const char *old,
                                   const char *new);
extern int        __librt_utime_r(struct _reent *r, const char *path,
                                  const struct utimbuf *times);
extern mode_t     __librt_umask_r(struct _reent *r, mode_t cmask);

extern int        __librt_mkdir_r(struct _reent *r, const char *path,
                                  mode_t mode);
...
extern char      *__librt_getcwd_r(struct _reent *r, char *buf,
                                   size_t size);
extern int        __librt_closedir_r(struct _reent *r, DIR *dirp);
extern DIR       *__librt_opendir_r(struct _reent *r,
                                    const char *dirname);
extern struct dirent *__librt_readdir_r(struct _reent *r, DIR *dirp);
extern int        __librt_readdir_r_r(struct _reent *r, DIR *dirp,
                                      struct dirent *entry,
                                      struct dirent **result);
extern void       __librt_rewinddir_r(struct _reent *r, DIR *dirp);
extern void       __librt_seekdir_r(struct _reent *r, DIR *dirp,
                                    long int loc);
extern long int   __librt_telldir_r(struct _reent *r, DIR *dirp);

extern int        __librt_isatty_r(struct _reent *r, int fd);

extern int        __librt_gettod_r(struct _reent *r,
                                   struct timeval *ptimeval,
                                   void *ptimezone);
extern int        __librt_settod_r(struct _reent *r,
                                   const struct timeval *ptimeval,
                                   const struct timezone *ptimezone);
extern int        __librt_usleep_r(struct _reent *r, useconds_t usec);
extern unsigned int __librt_sleep_r(struct _reent *r,
                                    unsigned int seconds);
extern clock_t    __librt_times_r(struct _reent *r, struct tms *buf);

extern caddr_t    __librt_sbrk_r(struct _reent *r, ptrdiff_t incr);

extern int        __librt_sys_lwmutex_create_r(
                      struct _reent *r, sys_lwmutex_t *lwmutex,
                      const sys_lwmutex_attr_t *attr);
extern int        __librt_sys_lwmutex_destroy_r(struct _reent *r,
                                                sys_lwmutex_t *lwmutex);
extern int        __librt_sys_lwmutex_lock_r(struct _reent *r,
                                             sys_lwmutex_t *lwmutex,
                                             unsigned long long timeout);
extern int        __librt_sys_lwmutex_trylock_r(struct _reent *r,
                                                sys_lwmutex_t *lwmutex);
extern int        __librt_sys_lwmutex_unlock_r(struct _reent *r,
                                               sys_lwmutex_t *lwmutex);

extern int main(int argc, const char *argv[]);

/* Constructor priority 104 runs ahead of user constructors so the
 * syscall table is populated before any static-init code touches
 * the filesystem, heap, or threads. */
