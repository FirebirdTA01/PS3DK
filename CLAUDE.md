...
Building a modern, feature-complete open-source PlayStation 3 SDK that supports **C++17 on both PPU (PowerPC 64) and SPU (Cell SPE)**. Replaces the stale ps3dev baseline (GCC 7.2.0 / binutils 2.22 / newlib 1.20.0) with current versions, fills PSL1GHT API gaps, ships fragment shader support, and uses the official Sony PS3 SDK the reference SDK privately as a coverage oracle (never committed, never shipped — see `reference/SONY_SDK_POLICY.md`).

## Authoritative plan

`~/.claude/plans/i-m-going-to-create-melodic-dawn.md` (per-host). Also mirrored at `docs/plan.md` in this repo for reference.

## Status snapshot

- **Phase 0 (bootstrap):** complete on Windows side. ~30 authored files: scripts, Rust tooling, samples, docs.
- **Phase 1a (binutils 2.42 dry-build):** in progress — blocked on Windows Git Bash lacking pacman/MinGW64 toolchain. Resumes natively on CachyOS.
...
| the reference SDK mount | Deferred — user provides path later. Phase 3 coverage tooling falls back to PSL1GHT-only mode until then. |
...
├── the reference SDK/          .gitignored read-only mount; never commit
...
User-specific memory at `~/.claude/projects/C--<...>--PS3-Custom-Toolchain/memory/` (the path differs per OS — see `docs/HANDOFF.md` for the CachyOS migration step). Index in `MEMORY.md` covers user role, communication preferences, decision style, project goals, locked decisions, hardware setup, repo paths, and PSDevWiki references.

## House rules

- Never commit to `the reference SDK/` (`.gitignore` enforces).
- Never copy the reference SDK code into the repo. Independent only.
