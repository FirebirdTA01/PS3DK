/*
 * PS3 Custom Toolchain — <sys/return_code.h> Sony-SDK compat shim.
 *
 * Sony samples test `cell*` return values against `CELL_OK` (= 0) and
 * the LV2 system libraries return `int32_t` status codes; those are
 * the only two symbols this header has ever provided in practice.
