 ...
 * layout, what the reference compiler emits) and PSL1GHT's rsx*Program runtime
 ...
 * -------------------------------------------------------------------- *
 *
 * the reference compiler and PSL1GHT cgcomp encode CgBinaryVertexProgram.
 * attributeOutputMask differently.  For a pass-through
 * POSITION + COLOR shader, the reference compiler writes 0x1 (HPOS only), PSL1GHT
 * cgcomp writes 0x5 (HPOS + COL0's dual-bit signal).  The rsx
 * runtime consumes the PSL1GHT encoding, so handing the reference compiler's raw
