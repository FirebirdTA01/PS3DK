 ...
 * Reads CgBinaryProgram blobs (the reference compiler output) directly and writes NV40
 * command words to the current gcm command buffer — no PSL1GHT
 * rsx{Vertex,Fragment}Program intermediate, no rsxLoad* forwarding.
 *
 * The reference compiler output is the source of truth; the hardware register encoding
                                             ...
                                             CGprogram prog, uint32_t offset)
{
     ...
     *              the reference compiler appears to encode output-register selection
     ...
     *              sample, the reference compiler's word 0 has 0x00 there vs
