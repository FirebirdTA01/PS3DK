 ...
 *
 * reference-SDK-style surface for the video-output subsystem (resolution
 ...
 *
 * The the reference SDK bundles this surface into <sysutil/sysutil_sysparam.h>;
 * our shim under the same path re-exports it so the reference SDK sample code
...
/* The the reference SDK names the component ordering; we alias onto PSL1GHT's enum. */
 ...
 * the reference SDK-sample field name through an anonymous union without
 ...
 *
 * Field-name mapping:
 *   .resolution   (PSL1GHT)  <-->  .resolutionId  (the reference SDK samples)
 *
 * Unified via `union { u8 resolution; u8 resolutionId; }` so the reference SDK
