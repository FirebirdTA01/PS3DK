...
int main(void)
{
    ...
    if (!device || !gl_context) {
        printf("hello-psgl-rotating-quad: PSGL init failed\n");
        psglExit();
        return 1;
    }
    ...
    if (!texture_rgba) {
        printf("hello-psgl-rotating-quad: texture decode failed\n");
        psglExit();
        return 1;
    }
    ...
    printf("hello-psgl-rotating-quad: ready\n");
    for (int i = 0; i < 900; i++) {
        GLfloat angle = 35.0f + (GLfloat)i * 0.75f;
    ...
    printf("hello-psgl-rotating-quad: done\n");
    psglExit();
    return 0;
}
