...
#define VP_PATH "/dev_hdd0/tmp/hello-psgl-textured-quad.vpo"
#define FP_PATH "/dev_hdd0/tmp/hello-psgl-textured-quad.fpo"
...
int main(void)
{
    ...
    if (!write_shaders()) {
        printf("hello-psgl-textured-quad: shader write failed\n");
        return 1;
    }
    ...
    if (!device || !gl_context) {
        printf("hello-psgl-textured-quad: PSGL init failed\n");
        psglExit();
        return 1;
    }
    ...
    tex_param = cgGetNamedParameter(fp, "diffuseMap");
    if (!cg_context || !vp || !fp || !mvp_param || !tex_param) {
        printf("hello-psgl-textured-quad: Cg setup failed err=%d\n", (int)cgGetError());
    ...
    if (!texture_rgba) {
        printf("hello-psgl-textured-quad: texture decode failed\n");
        cgDestroyContext(cg_context);
        psglExit();
        return 1;
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei)texture_width,
                 ...
                 texture_rgba);
    free(texture_rgba);
    cgGLSetTextureParameter(tex_param, texture);
    ...
    printf("hello-psgl-textured-quad: ready\n");
    for (int i = 0; i < 900; i++) {
    ...
    printf("hello-psgl-textured-quad: done\n");
    cgDestroyContext(cg_context);
    psglExit();
    return 0;
}
