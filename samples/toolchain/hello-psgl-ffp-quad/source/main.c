...
int main(void)
{
    ...
    if (!device || !gl_context) {
        printf("hello-psgl-ffp-quad: PSGL init failed\n");
        psglExit();
        return 1;
    }
    ...
    if (!texture_rgba) {
        printf("hello-psgl-ffp-quad: texture decode failed\n");
        psglExit();
        return 1;
    }
    ...
    printf("hello-psgl-ffp-quad: ready\n");
    for (int i = 0; i < 900; i++) {
    ...
    printf("hello-psgl-ffp-quad: done\n");
    psglExit();
    return 0;
}
