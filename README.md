...
| PSL1GHT (v3) | main | PPU + SPU runtime | Homebrew runtime rebased for the reference SDK-style naming; source-compat shim retained for legacy homebrew |
...
- **PSL1GHT v3**: the reference SDK-style naming (`cellXxx`, `CELL_XXX_*`, `CellXxx`) with a compatibility shim that keeps legacy homebrew building for 1–2 releases.  Fragment-shader-capable RSX (NV40-FP assembler first, full Cg compiler at `tools/rsx-cg-compiler/`).
