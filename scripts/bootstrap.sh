...
#   5. Document the the reference SDK mount step (does not execute it automatically —
...
# -----------------------------------------------------------------------------
# 4. the reference SDK mount (informational — not executed).
# -----------------------------------------------------------------------------
say "=== the reference SDK mount (deferred) ==="

if [[ -e "$PS3_TOOLCHAIN_ROOT/the reference SDK" ]]; then
    say "the reference SDK/ already exists (mounted or placeholder)."
else
    cat <<EOF
...
It is never committed and never shipped. When ready to mount it:
  ...
  # Windows (MSYS2):
  cmd //c mklink /J "$(cygpath -w "$PS3_TOOLCHAIN_ROOT/the reference SDK")" "D:\\path\\to\\Sony\\SDK\\the reference SDK"
  icacls "$(cygpath -w "$PS3_TOOLCHAIN_ROOT/the reference SDK")" /deny "\$USERNAME":(WD,DC)
