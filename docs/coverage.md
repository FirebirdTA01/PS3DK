...
Sony reference SDK the reference SDK symbols vs our install tree.  Diff is by symbol name — PSL1GHT-style renamed wrappers (e.g. `sysThreadGetId` vs Sony's `sys_ppu_thread_get_id`) will show as missing here, even though a functional equivalent exists.  A future pass with a curated alias table will fold those in.
