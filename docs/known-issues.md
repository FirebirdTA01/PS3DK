...
1. New `sdk/libdbgfont/` directory with the font data + shaders
   ...
   the reference compiler (same path the reference samples use) or our own
...
## reference-SDK-compat struct-field aliases live in SDK, not in PSL1GHT
...
canonical `offset | (type << 24)` from the reference the reference SDK's
`gcm_methods.h`.
