...
| `pthread-sync` | POSIX pthread shim over Lv-2 primitives: static mutex/cond/once initialisers, recursive mutexes, `pthread_mutex_trylock` POSIX `EBUSY`, thread-specific data, `pthread_create`/`join`, `pthread_self`/`equal`, and condition signalling; requires `PTHREAD_OK` |
