...
- librsx it links: **the installed `$PS3DK/ppu/lib/librsx.a` is built from `sdk/librsx/` (the native multilib SUBLIB), NOT from `src/ps3dev/PSL1GHT/ppu/librsx/`** — verified 2026-08-27 by disassembling `rsxInit` in the installed archive (it carries sdk's 2-word `NV04_CONTEXT_SURFACES_2D_DMA_IMAGE_DESTIN=FRAME_BUFFER` init emission, which the PSL1GHT copy lacks). **`sdk/librsx/src/commands_impl.h` is NOT an identical copy** (stale note corrected 2026-08-27): it is the native multilib SUBLIB (`74a1eec`) and carries deliberate reference-alignment fixes on top of the PSL1GHT copy — `2baf898` (WAIT_FOR_IDLE after clear, per-instruction VP upload, `SetReportLocation`). Both are post-f9992c9 for `SetTransferImage`. When aligning `SetTransferImage`, apply the same change to both files by hand — **do not `cp` one over the other** or the sdk/ fixes are clobbered.
...
**#2 — RESOLVED 2026-08-27 (root cause found; fix pending, task t_08c60dd3).** `rsx/rsx_function_macros.h` `rsxContextCallback` wraps its `bctrl` into `context->callback` in an inline asm whose clobber list is only `r31, r0, lr` — no `"memory"`, no volatile registers. GCC ≥ 11 `ipa-modref` proves the function writes no memory, so every `RSX_FUNC` keeps `context->current` cached across the wrap callback and stores through the stale pointer (objdump of the installed `librsx.a`, `rsxSetClearColor`: stores via pre-callback `r28`, sets `current = old+count`, never reloads `8(context)`). RPCS3's HLE callback hops 32KB fragments; contiguity hides the bug for 31 hops, the wrap to fragment 0 writes past `io+1MB` and the RSX parks past the buffer. Old PSL1GHT + GCC 7/8 (no modref) works; this would also fail on hardware — it is NOT an RPCS3 quirk and NOT `CB_SIZE`. Fix: pin r3/r4, add `"memory"` + r5–r12/ctr/cr/xer clobbers (sdk header + patch for the vendored copy); verify by objdump reload after the `bl`. §5(C) is therefore no longer needed for the hang, though the reference callback contract is still worth reading. Original notes kept below for history. **Also found 2026-08-27 while validating:** blitting's exit path (`bitmapDestroy → rsxFree`) faulted in `heapFree` — a separate pre-existing bug in OUR `runtime/lv2/librt/heap.c` rewrite (wrong coalesced size, missing bounds checks), reproducible with `rsxInit → rsxMemalign → rsxFree` and no RSX commands; fixed alongside (task `t_acb016c5`). Note the installed `librt.a` comes from `runtime/lv2/librt/`, not the vendored PSL1GHT `librt`.
