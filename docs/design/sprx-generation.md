...
| Signing | `prx-gen` emits a raw ELF `.prx`; `make_sprx` (geohot `make_self.c` compiled with `-DSPRX`: apptype/authid/`s_flags=7`, no section table) produces `.sprx`. | RPCS3 loads the unsigned `.prx` when the booted main executable is a raw `.elf` (memo §4.3) — format bugs and signing bugs are debugged separately, in that order. |
...
| A4 | `make_sprx` output loaded from a `.fake.self`-booted importer | A1 repeated on the `.sprx` |
